...
my $cds_file;
my $multi_fasta_mode;
my $outputfolder = 'default_output_folder';
...
if (-d "SNPGenie_Results") { # Can also use "./$outputfolder"; use "-e" to check file
	#die "\n\n## WARNING:\n## The directory $outputfolder already exists in the ".
		...
		while (<INFILE>) {
			if (/>/) {
				if($seen_seq == 0) {
					...
					die "\n\n### WARNING: There are multiple sequences in $the_fasta_file.\n".
...
if (-e "/segmentdir/A_GUANGDONG_17SF003_2016_NS.vcf") {
	print ("vcf exists!\n");}
else {
	print ("vcf does not exist at "); print (getcwd());}
print "\n\n################################################################################".
	"\n##                                                                            ##".
	"\n##                             SNPGenie Initiated!                            ##".
	"\n##                                                                            ##".
...
#				my $fasta_nt = $seq_by_index_arr[$position-1];
				...
				my $fasta_window = substr($seq, ($position-5), 10); #JLL
				...
				if($snp_report_reference_nt ne $fasta_nt) {
					...
					print "\n## WARNING: In $file_nm, $curr_product, site $position,\n".
						...
						"Sequence at issue: $fasta_window, Expected nt: $snp_report_reference_nt, recieved nt: $fasta_nt\n"; #JLL
				}
			}
		}
				...
				if(length($curr_partial_codon) == 3) { # once $curr_partial_codon reaches 3-nt
					...
					if ( $i == 1 && # FIRST segment
						...
						print "\n## WARNING: Please be aware that the product ".
								...
								"\nPosition data: start site: $start_site, stop site: $stop_site, this nt: $this_nt, j = $j\n". #JLL
								"Coding length sum: $coding_length_sum, coding sequence:\n$coding_seq\n". #JLL
								"Start Key: $this_start_key, Stop key: $this_stop_key, coding length: $coding_length"; #JLL
...
#				}
		...
		# Go back to working directory, in which $outputfolder resides
...
#						}
					...
					#} elsif($format_value =~ /AD/) { # We've got a VCF of POOL; we need AD and DP
						...
						if($sample_value_arr[$colon_count_before_AD] =~ /(\d+)\,(\d+)/) { # REF and 1 ALT
							$AD1 = $1;
							$AD2 = $2;
						} elsif ($sample_value_arr[$colon_count_before_AD] =~ /(\d+)/) {#Just 1 ALT
							$AD1 = $sample_value_arr[$colon_count_before_RD];   ###ASSUMING that 1 ALT = RD, ALT format
							$AD2 = $1;
						}
...
sub sliding_window {
	...
	if (-e "$new_file_name") { # Can also use "./$outputfolder"; use "-d" to check directory
...
sub end_the_program {
	...
	print "\n################################################################################".
		...
		"\n##             Please find results in the $outputfolder folder.            ##\n".
		"################################################################################".
		"\n\n\n"; 
}