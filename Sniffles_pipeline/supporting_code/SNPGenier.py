...
def SNPgenier(runCFG, vcffolder, slidingwindow, threads=mp.cpu_count()):
	#set parameters
	outDir = os.path.join(runCFG['exec']['outdir'], 'SNPGenie_output')
	logfile = runCFG['exec']['logfile']

	#set reference sequence
	reference_sequence_path = os.path.join(os.getcwd(),runCFG['exec']['referenceSequence'])
	...
	vcffiles = glob.glob(vcffolder + "/*.vcf")
	if len(vcffiles) > 1:
		if "all_snps.vcf" not in vcffiles:
			master_vcf_name = VCFcombiner(runCFG, vcffiles, "all_snps.vcf")
	else:
		master_vcf_name = vcffiles[0]
		...
		with open(runCFG['postprocessing']['gtfFileName'], 'r') as master_gtf:
			...
			with open (os.path.join(vcffolder, master_vcf_name), 'r') as master_vcf:
				...
				for fastaseq in refseqs:
					...
					cmd = (" ".join(("perl", "snpgenie.pl", "--vcfformat=4", "--snpreport=" + segmentdir +"/" + segname + ".vcf", "--fastafile="+ segmentdir +"/" +  segname + ".fasta", "--gtffile="+ segmentdir +"/" + segname + ".gtf", "--outputfolder="+segmentdir, "--minfreq="+str(runCFG['postprocessing']['minSNPfreq'])+"\'")))
