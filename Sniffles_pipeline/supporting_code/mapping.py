...
def mapping(runCFG,param_paths,outDir,threads='8'):
	...
	for param_path in param_paths:
		...
		if runCFG['exec']['unpaired']:
			...
			mergecmd = f" && samtools merge /output/{id}.bam /output/{id}_unpaired.bam /output/{id}_paired.bam\'"
	...
	with open(os.path.join(outDir,'average_depth.log'),'w') as outdepth:
		...
		for bam in bam_list:
			...
			with open(logfile,'a') as outlog:
				...
				cmd = f'bash -c "samtools view /indata/{filename} > {id}.tmp.sam  && /tools/bbmap/pileup.sh in={id}.tmp.sam out={id}_coverage.tsv ref=/reference/{reference_sequence_name} 32bit=t && rm {id}.tmp.sam"'
