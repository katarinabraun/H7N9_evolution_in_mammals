...
def snpcaller(runCFG,bam_files,threads='1'):
	...
	for bam, sample in zip(bams, sample_list):
		#mpileup command
		outlogHeader = f"\"{bam.split('.')[0]}\n-----------\n\""
		cmd1 = f'bash -c \'printf {outlogHeader} >> {os.path.join("/logfile", os.path.basename(logfile))} && samtools mpileup -ABR -d 1000000 {bam} -f /ref/{reference_sequence_name} > {sample}.mpileup'

		#varscan command
		cmd2 = f'java -jar /tools/varscan.jar mpileup2snp {sample}.mpileup --min-coverage {min_cov} --min-avg-qual {snp_qual_threshold} --min-var-freq {snp_frequency} --strand-filter 1 --output-vcf 1 --variants --vcf-sample-list <(echo -e "{sample}") > {sample}.vcf\''
